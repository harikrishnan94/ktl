...
//
// Copyright (c) 2022 by Alain Espinosa. (c) 2023 by Harikrishnan Prabakaran
/////////////////////////////////////////////////////////////////////////////////
...
#endif
static constexpr inline void _wymum(uint64_t* A, uint64_t* B) noexcept {
#if WYHASH_32BIT_MUM
...
constexpr static inline uint64_t _wymix(uint64_t A, uint64_t B) noexcept {
    _wymum(&A, &B);
    return A ^ B;
}
...
// read functions
#if WYHASH_LITTLE_ENDIAN
static inline uint64_t _wyr8(const uint8_t* p) noexcept {
    uint64_t v;
    memcpy(&v, p, 8);
    return v;
}
static inline uint64_t _wyr4(const uint8_t* p) noexcept {
    uint32_t v;
    memcpy(&v, p, 4);
    return v;
}
...
#endif
static inline uint64_t _wyr3(const uint8_t* p, size_t k) noexcept {
...
constexpr static inline uint64_t wyhash64(uint64_t A, uint64_t B) noexcept {
...
static inline uint64_t wyrand(uint64_t* seed) noexcept {
    *seed += 0xa0761d6478bd642full;
    return _wymix(*seed, *seed ^ 0xe7037ed1a0b428dbull);
}

#if !WYHASH_32BIT_MUM
// fast range integer random number generation on [0,k) credit to Daniel Lemire. May not work when
// WYHASH_32BIT_MUM=1. It can be combined with wyrand, wyhash64 or wyhash.
static inline uint64_t wy2u0k(uint64_t r, uint64_t k) noexcept {
    _wymum(&r, &k);
    return k;
}
#endif
...
struct rand {
    ...
    /// </summary>
    /// <param name="seed">The only value needed to generate the same sequence of pseudo-random
    /// numbers</param>
    ...
    inline std::vector<T> generate_stream(size_t size) noexcept {
        std::vector<T> result;
    ...
    /// <param name="size">The number of elements of the vector to generate</param>
    template<class T = uint8_t>
    void generate_stream(std::vector<T>& vec, size_t size) noexcept {
        size_t sizeOf64 = (size * sizeof(T) + sizeof(uint64_t) - 1)
            / sizeof(uint64_t);  // The number of 64-bits numbers to generate
...
namespace internal {
    ...
    struct hash_imp {
            ...
            for (size_t i = 0; i < 4; i++) {
                ...
                do {
                    ...
                    for (size_t j = 0; j < i; j++) {
                        ...
                        if (std::popcount(secret[j] ^ secret[i]) != 32) {
                            ok = 0;
                            break;
                        }
#elif defined(__GNUC__) || defined(__INTEL_COMPILER) || defined(__clang__)
        ...
        constexpr inline uint64_t wyhash(const uint8_t* data, size_t len) const noexcept {
            // code taken from 'wyhash.h::wyhash(const void* key, size_t len, uint64_t seed, const
            // uint64_t * secret)' with seed=0
        ...
        /// <returns>A 64-bits hash</returns>
        constexpr inline uint64_t wyhash(uint64_t number) const noexcept {
    ...
    /// </summary>
    /// <typeparam name="STRING_TYPE">The type of the string, ex: std::string, std::wstring,
    /// ...</typeparam>
    template<class STRING_TYPE>
    struct hash_string_base: private hash_imp {
        using hash_imp::hash_imp;  // Inherit constructors
        inline uint64_t operator()(const STRING_TYPE& elem) const noexcept {
            return hash_imp::wyhash(
                reinterpret_cast<const uint8_t*>(elem.data()),
                sizeof(STRING_TYPE::value_type) * elem.size());
        }
    };
};  // namespace internal
...
template<class T>
struct hash: private internal::hash_imp {
    using hash_imp::hash_imp;  // Inherit constructors
...
template<class T>
struct hash<T*>: private internal::hash_imp {
    using hash_imp::hash_imp;  // Inherit constructors
    inline uint64_t operator()(const T* elem) const noexcept {
        static_assert(sizeof(T) > 0, "Type to hash T should have variables");
        return hash_imp::wyhash(reinterpret_cast<const uint8_t*>(elem), sizeof(T));
...
template<>
struct hash<uint64_t>: private internal::hash_imp {
    using hash_imp::hash_imp;  // Inherit constructors
...
template<>
struct hash<int64_t>: private internal::hash_imp {
    using hash_imp::hash_imp;  // Inherit constructors
...
// C strings
// template<>
// struct hash<char*>: private internal::hash_imp {
//     using hash_imp::hash_imp;  // Inherit constructors
...
// };
// template<>
// struct hash<const char*>: private internal::hash_imp {
//     using hash_imp::hash_imp;  // Inherit constructors
//     inline uint64_t operator()(const char* data) const noexcept {
//         return hash_imp::wyhash(reinterpret_cast<const uint8_t*>(data), strlen(data));
//     }
...
// template<>
// struct hash<std::string>: public internal::hash_string_base<std::string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::wstring>: public internal::hash_string_base<std::wstring> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::u16string>: public internal::hash_string_base<std::u16string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::u32string>: public internal::hash_string_base<std::u32string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
...
// template<>
// struct hash<std::string_view>: public internal::hash_string_base<std::string_view> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::wstring_view>: public internal::hash_string_base<std::wstring_view> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::u16string_view>: public internal::hash_string_base<std::u16string_view> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::u32string_view>: public internal::hash_string_base<std::u32string_view> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// #endif
...
// template<>
// struct hash<std::pmr::string>: public internal::hash_string_base<std::pmr::string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::pmr::wstring>: public internal::hash_string_base<std::pmr::wstring> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::pmr::u16string>: public internal::hash_string_base<std::pmr::u16string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::pmr::u32string>: public internal::hash_string_base<std::pmr::u32string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// #endif
...
// #if __cpp_char8_t
// template<>
// struct hash<std::u8string>: public internal::hash_string_base<std::u8string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::u8string_view>: public internal::hash_string_base<std::u8string_view> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// template<>
// struct hash<std::pmr::u8string>: public internal::hash_string_base<std::pmr::u8string> {
//     using hash_string_base::hash_string_base;  // Inherit constructors
// };
// #endif
};  // namespace wy
...
struct hash<T> {
    ...
    inline uint64_t operator()(const T& elem) const noexcept {
        static_assert(sizeof(T) > 0, "Type to hash T should have variables");
        wy::internal::hash_imp imp;
        return imp.wyhash(static_cast<uint64_t>(elem));
    }
};
}  // namespace std
